// Driver Register (src/app/Frontend/Driver/Auth/Register/page.tsx)
'use client';
...
export default function DriverRegister() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    const v = DriverRegisterSchema.safeParse(form);
    if (!v.success) {
    ...
    try {
      await apiClient.post('/auth/driver/register', {
        email: form.email,
        username: form.username,
        password: form.password,
      }, { withCredentials: true });
      setStatus('success');
      setTimeout(() => router.push('/Frontend/Driver/Feed'), 1000);
    } catch (err: unknown) {
      setStatus('idle');
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-md bg-white p-8 shadow-md rounded-xl space-y-6">
        <h2 className="text-center text-2xl font-bold">Driver Register</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {status === 'success' && <p className="text-green-600">Registered! Redirecting…</p>}
