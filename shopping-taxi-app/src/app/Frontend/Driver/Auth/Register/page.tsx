...
export default function DriverRegisterPage() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const validation = DriverRegisterSchema.safeParse(form);
    ...
    try {
      ...
      if (isAxiosError(err)) {
        ...
        setError(axiosErr.response?.data.error ?? 'Driver registration failed.');
      } else {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-md space-y-6 bg-white p-8 shadow-md rounded-xl">
        <h2 className="text-center text-2xl font-bold">Driver Registration</h2>
        ...
        <p className="text-center text-sm text-gray-600">
          ...
          <Link href="/Driver/Auth/Login" className="text-blue-600 hover:underline">Log in</Link>
        </p>
      </div>
    </main>
  );
